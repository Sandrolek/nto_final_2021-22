...
FLY_HEIGHT = 0.8        # Высота полета при движении по линии

#mask = ((32, 45, 128), (65, 189, 255))
mask = ((20, 120, 0), (40, 140, 255))   # Диапозон по HSV цвета для маски изображения линии

k = 0.25 / 2300         # Коэффициент для правильного подсчета площади

SPEED_X = 0.08          # Скорость по оси X при движении по линии

defects = []            # Массив с координатами повреждений 
...
def home(xy):     # функция полета в место старта и посадка
    s = list(map(float, xy)) 
...
def convert_angle(angle=0, w_min=320, h_min=240): # Правильная интерпретация угла наклона описанного прямоугольника
    if angle < -45:
        angle = 90 + angle
...
def get_qr():  # Получение данных с Qr кода
    qr = None
    while qr == None:
...
def get_rect_top(img, orig): # Поиск прямоугольника в верхней части изображения  
    cnts, _ = cv2.findContours(img.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)      # Отрисовка контуров
    cnts.sort(key=cv2.minAreaRect)          # Сортировка контуров по площади
    (x_min, y_min), (w_min, h_min), angle = (0, 0), (0, 0), 0
    if len(cnts) > 0:
        cnt = max(cnts, key=cv2.contourArea)
        if cv2.contourArea(cnt) > 100:          # Если  уконтура площадь меньше заданной то мы ее игнорируем
            #state = 
            rect = cv2.minAreaRect(cnt)         # Получение прямоугольника минимальной площади
...
def img_cb(data): # callback функция для следования по линии
    global state

    cv_image = bridge.imgmsg_to_cv2(data, 'bgr8')  # Получение изображения

    hsv = cv2.cvtColor(cv_image, cv2.COLOR_BGR2HSV)     # Перевод изображения с камеры в HSV
    black = cv2.inRange(hsv, mask[0], mask[1])          # Применение маски для выделения линии по цвету
    #cv2.imshow("BW image", black)
    top = black[:(HEIGHT // 2), :]          # Обрезание верхней части изображения с камеры    
    ...
    if is_full: # just line
        ...
        telem = get_telemetry(frame_id="aruco_map")   #телеметрия относительно aruco_map          
...
def defect_cb(data): # callback функция для поиска разломов на трубопроводе
    global defects
...
def rotate_to_line(): #Поворот на линию на стартовой позиции
...
print("Taking off")
navigate(x=0, y=0, z=1, frame_id="body", speed=0.7, auto_arm=True) # взлет по body
rospy.sleep(4)
# telem = get_telemetry(frame_id="aruco_map")
# navigate_wait(x=telem.x, y=telem.y, z=telem.z-0.3, frame_id="aruco_map", speed=1, auto_arm=False)
# rospy.sleep(3)

# Получение координат старта
...
navigate_wait(x=start_line[0], y=start_line[1], z=FLY_HEIGHT, speed=0.3, frame_id="aruco_map") # Полет к старту
rospy.sleep(2)
...
# Создание подписчиков на топики
